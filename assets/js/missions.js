...
{ id:'m1', tour:'Getting your bearings', goal:'Find out which directory you are standing in.', par:1,
  check:(sh,l)=>used(l,/^pwd\s*$/), hint:'Print Working Directory — three letters.', teach:'pwd' },
{ id:'m2', tour:'Getting your bearings', goal:'List the visible contents of your home directory.', par:1,
  check:(sh,l)=>used(l,/^ls\b/) && (sh.cwd==='/home/student' || /~|\/home\/student/.test(l)),
  hint:'Just: ls', teach:'ls' },
...
{ id:'m6', tour:'Getting your bearings', goal:'Go back to the directory you were in before this one.', par:1,
  check:(sh,l)=>used(l,/^cd\s+-\s*$/), hint:'A single dash means "back where I came from".', teach:'cd_prev' },
{ id:'m7', tour:'Getting your bearings', goal:'Move up one level to the parent directory.', par:1,
  check:(sh,l)=>used(l,/^cd\s+\.\.\/?\s*$/), hint:'Two dots mean "up one".', teach:'cd_up' },
{ id:'m8', tour:'Getting your bearings', goal:'Jump straight home again, without typing any path.', par:1,
  check:(sh,l)=>used(l,/^cd(\s+~)?\s*$/) && sh.cwd==='/home/student', hint:'cd with nothing after it.', teach:'cd_home' },
...
{ id:'m18', tour:'Making and breaking', goal:'Copy Documents/notes.txt into archive/, showing what happens as it copies.', par:1,
  check:(sh,l)=>!!sh.node('/home/student/archive/notes.txt') && cmdIs(l,'cp'),
...
{ id:'m21', tour:'Making and breaking', goal:'Make an empty directory called junk, then delete it with the safe command that only removes empty directories.', par:2,
  check:(sh,l)=>used(l,/^rmdir\s/) && /junk/.test(l) && !sh.node('/home/student/junk'),
...
{ id:'m25', tour:'Locking things down', goal:'Show your own user ID, group ID and group memberships.', par:1,
  check:(sh,l)=>used(l,/^id\s*$/), hint:'Two letters.', teach:'id' },
...
{ id:'m34', tour:'Locking things down', goal:'Display the permission mask used for newly created files.', par:1,
  check:(sh,l)=>used(l,/^umask\s*$/), hint:'One word, no arguments.', teach:'umask' },

/* ---- tour 5: processes ---- */
{ id:'m35', tour:'Wrangling processes', goal:'List every process running on the system, in the verbose user-oriented layout.', par:1,
  check:(sh,l)=>used(l,/^ps\s+(aux|auxw|axu|-ef|-eF)\b/), hint:'BSD style — no dash: ps aux', teach:'ps_aux' },
...
{ id:'m38', tour:'Wrangling processes', goal:'Bring job number 1 back into the foreground.', par:1,
  check:(sh,l)=>used(l,/^fg(\s+%1)?\s*$/), hint:'fg %1 — the percent sign means "job", not PID.', teach:'fg' },
{ id:'m39', tour:'Wrangling processes', goal:'List every signal your system supports.', par:1,
  check:(sh,l)=>used(l,/^kill\s+-l\s*$/), hint:'kill -l  (that is a lowercase L for list).', teach:'kill_l' },
{ id:'m40', tour:'Wrangling processes', goal:'Start xload in the background again, then terminate it with the unignorable KILL signal by job number.', par:2,
  check:(sh,l)=>used(l,/^kill\s+-(9|KILL|SIGKILL)\s+%?\d+/) || used(l,/^kill\s+-s\s+(9|KILL|SIGKILL)\s+%?\d+/),
  hint:'xload &   then   kill -9 %1', teach:'kill_9' },
...
{ id:'m47', tour:'Reading the manual', goal:'Search all manual descriptions for anything to do with "directory".', par:1,
  check:(sh,l)=>used(l,/^(apropos\s+directory|man\s+-k\s+directory)\s*$/), hint:'apropos directory — or man -k directory.', teach:'apropos' },
{ id:'m48', tour:'Reading the manual', goal:'Get the one-line description of the wc command.', par:1,
  check:(sh,l)=>used(l,/^whatis\s+wc\s*$/), hint:'whatis wc', teach:'whatis' },
{ id:'m49', tour:'Reading the manual', goal:'List every alias currently defined in this shell.', par:1,
  check:(sh,l)=>used(l,/^alias\s*$/), hint:'Just: alias', teach:'alias_list' },
...
{ id:'m51', tour:'The environment', goal:'Print the path of your home directory by expanding a variable.', par:1,
  check:(sh,l)=>used(l,/^echo\s+["']?\$\{?HOME\}?["']?\s*$/), hint:'echo $HOME', teach:'echo_home' },
{ id:'m52', tour:'The environment', goal:'List all of the active environment variables.', par:1,
  check:(sh,l)=>used(l,/^(env|printenv)\s*$/), hint:'Three letters.', teach:'env' },
{ id:'m53', tour:'The environment', goal:'Install the vim package on this Fedora system.', par:1,
  check:(sh,l)=>used(l,/^sudo\s+dnf\s+install\s+(-y\s+)?vim\s*$/) || used(l,/^sudo\s+dnf\s+install\s+vim\s+-y\s*$/),
  hint:'sudo dnf install vim', teach:'dnf_install' },
{ id:'m54', tour:'The environment', goal:'Switch bash command-line editing into Vim mode.', par:1,
  check:(sh,l)=>used(l,/^set\s+-o\s+vi\s*$/), hint:'set -o vi', teach:'set_vi' },
{ id:'m55', tour:'The environment', goal:'You accidentally cat-ed a binary and the screen is garbage. Fix the terminal.', par:1,
  check:(sh,l)=>used(l,/^reset\s*$/), hint:'One word: reset', teach:'reset' },
...
{ id:'m63', tour:'Lab 01', goal:'Use grep to find which line in network.conf sets the port.', par:1,
  check:(sh,l)=>(used(l,/^grep\s+.*port\s+.*network\.conf/) || used(l,/^grep\s+.*network\.conf.*port/) || used(l,/^grep\s+.*(3000|port)\b.*network\.conf/)),
...
{ id:'m66', tour:'Lab 01', goal:'Verify: use grep to confirm the port is now 8080.', par:1,
  check:(sh,l)=>used(l,/^grep\s+.*(8080|port)\s+.*network\.conf/) || used(l,/^grep\s+.*network\.conf.*(8080|port)/),
  hint:'grep 8080 network.conf', teach:'grep' },
{ id:'m67', tour:'Lab 01', goal:'Count how many lines are in network.conf.', par:1,
  check:(sh,l)=>cmdIs(l,'wc') && used(l,/network\.conf/),
...
{ id:'m82', mod:2, tour:'Staging & committing', goal:'Commit the staged changes with the message "initial commit".', par:1,
  check:(sh,l)=>used(l,/^git\s+commit\s+(-[a-z]*m\s|--message[\s=])/),
...
{ id:'m83', mod:2, tour:'Staging & committing', goal:'Stage and commit all tracked files in one step with a message.', par:1,
  check:(sh,l)=>used(l,/^git\s+commit\s+-(am|a\s+-m|ma)\b/),
...
{ id:'m85', mod:2, tour:'Branching & merging', goal:'List all local branches.', par:1,
  check:(sh,l)=>used(l,/^git\s+branch(\s+--list|\s+-l)?\s*$/),
...
{ id:'m91', mod:2, tour:'Remotes & collaboration', goal:'List your configured remote names.', par:1,
  check:(sh,l)=>used(l,/^git\s+remote(\s+-v)?\s*$/),
...
{ id:'m97', mod:2, tour:'History & diffs', goal:'Show the commit log with an ASCII branch graph.', par:1,
  check:(sh,l)=>used(l,/^git\s+log\b/) && /--graph/.test(l),
...
{ id:'m100', mod:2, tour:'Stashing & tagging', goal:'Stash your uncommitted changes to get a clean working directory.', par:1,
  check:(sh,l)=>used(l,/^git\s+stash(\s+push|\s+save)?\s*$/),
