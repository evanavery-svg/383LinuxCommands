...
/* ---- tour 8: Lab 01 ---- */
{ id:'m56', tour:'Lab 01', goal:'Enter the lab-01 directory.',
  check:sh=>sh.cwd==='/home/student/lab-01', hint:'cd lab-01', teach:'cd' },
{ id:'m57', tour:'Lab 01', goal:'List the files in lab-01 in long format to see their permissions.',
  check:(sh,l)=>cmdIs(l,'ls') && flag(l,'l') && sh.cwd==='/home/student/lab-01',
  hint:'ls -l', teach:'ls_l' },
{ id:'m58', tour:'Lab 01', goal:'Read the contents of broken_backup.sh — something is wrong on line 1.',
  check:(sh,l)=>used(l,/^cat\s+.*broken_backup\.sh/), hint:'cat broken_backup.sh', teach:'cat' },
{ id:'m59', tour:'Lab 01', goal:'The shebang says "bsh" instead of "bash". Fix it in place with sed.',
...
{ id:'m60', tour:'Lab 01', goal:'The script still is not executable. Add execute permission for the owner.',
...
{ id:'m61', tour:'Lab 01', goal:'Verify line 1 of broken_backup.sh now starts with #!/bin — show just the first line.',
  check:(sh,l)=>used(l,/^head\s+(-n\s*1|-1)\s+.*broken_backup\.sh/),
  hint:'head -n 1 broken_backup.sh', teach:'head' },
{ id:'m62', tour:'Lab 01', goal:'Before editing network.conf, back it up: copy it to network.conf.bak.',
  check:sh=>!!sh.node('/home/student/lab-01/network.conf.bak'),
  hint:'cp network.conf network.conf.bak', teach:'cp' },
{ id:'m63', tour:'Lab 01', goal:'Use grep to find which line in network.conf sets the port.',
  check:(sh,l)=>used(l,/^grep\s+.*port\s+.*network\.conf/),
  hint:'grep port network.conf', teach:'grep' },
{ id:'m64', tour:'Lab 01', goal:'Change the port from 3000 to 8080 in network.conf, editing in place.',
...
{ id:'m65', tour:'Lab 01', goal:'Change max_retries from 50 to 3 in network.conf, editing in place.',
...
{ id:'m66', tour:'Lab 01', goal:'Verify: use grep to confirm the port is now 8080.',
  check:(sh,l)=>used(l,/^grep\s+.*8080\s+.*network\.conf/),
  hint:'grep 8080 network.conf', teach:'grep' },
{ id:'m67', tour:'Lab 01', goal:'Count how many lines are in network.conf.',
  check:(sh,l)=>cmdIs(l,'wc') && flag(l,'l') && used(l,/network\.conf/),
  hint:'wc -l network.conf', teach:'wc' },
{ id:'m68', tour:'Lab 01', goal:'Pipe ls /var/log into wc -l to count how many entries the log directory holds.',
  check:(sh,l)=>used(l,/^ls\s+.*\/var\/log\s*\|\s*wc\s+-l/),
  hint:'ls /var/log | wc -l', teach:'pipe' },
{ id:'m69', tour:'Lab 01', goal:'Use a pipe: run ps aux and pipe it into grep to find sshd.',
  check:(sh,l)=>used(l,/^ps\s+aux\s*\|\s*grep\s+sshd/),
  hint:'ps aux | grep sshd', teach:'pipe' },
{ id:'m70', tour:'Lab 01', goal:'Write the text "Lab 01 complete" into a new file called report.txt using output redirection.',
...
{ id:'m71', tour:'Lab 01', goal:'Run sysinfo.sh and send its output into a file called sysinfo_output.txt.',
  check:sh=>!!sh.node('/home/student/lab-01/sysinfo_output.txt'),
  hint:'./sysinfo.sh > sysinfo_output.txt', teach:'redir' },
{ id:'m72', tour:'Lab 01', goal:'Use printf with command substitution to print: User: student',
  check:(sh,l)=>used(l,/^printf\b/) && /\$\(whoami\)/.test(l),
  hint:'printf \'User: %s\\n\' "$(whoami)"', teach:'printf' },
{ id:'m73', tour:'Lab 01', goal:'Everything is fixed. Run the repaired broken_backup.sh to prove it works.',
